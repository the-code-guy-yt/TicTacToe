class Board:
    ...
    def render(self):
        ...
        board_str = spacer
        board_str += (
            f" {self.render_cell(7)} | {self.render_cell(8)} | {self.render_cell(9)}\n"
        )
        board_str += spacer
        board_str += splitter
        board_str += spacer
        board_str += (
            f" {self.render_cell(4)} | {self.render_cell(5)} | {self.render_cell(6)}\n"
        )
        board_str += spacer
        board_str += splitter
        board_str += spacer
        board_str += (
            f" {self.render_cell(1)} | {self.render_cell(2)} | {self.render_cell(3)}\n"
        )
        board_str += spacer
